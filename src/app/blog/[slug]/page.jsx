...
export default async function BlogDetailPage({ params }) {
  ...
  <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-gray-900 mb-6 sm:mb-8">
    {blog.title}
        ...
        {/* Image + Initial Text */}
        <div className="grid grid-cols-1 md:grid-cols-[1.4fr_1fr] gap-6 md:gap-5 items-start">
          {/* Text beside image */}
          <div>
            {blog.content
              .trim()
          ...
          {blog.image && (
            <div className="hidden md:flex justify-end">
              <img
                src={blog.image}
                alt={blog.title}
                className="w-full max-w-[600px] md:max-w-[600px] rounded-xl shadow-md object-cover"
              />
            </div>
          )}
        </div>
